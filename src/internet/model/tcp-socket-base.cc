...
namespace ns3
{
                                   ...
                                   const uint32_t tcpPayloadSize)
  {
    ...
    else if (tcpPayloadSize > 0 && OutOfRange(seq, seq + tcpPayloadSize))
    {
      ...
      NS_LOG_WARN("At state " << TcpStateName[m_state] << " received packet of seq [" << seq << ":" << seq + tcpPayloadSize << ") out of range [" << m_tcb->m_rxBuffer->NextRxSequence() << ":" << m_tcb->m_rxBuffer->MaxRxSequence() << ")");
  ...
  TcpSocketBase::PeerClose(Ptr<Packet> p, const TcpHeader &tcpHeader)
  {
    ...
    if (tcpHeader.GetSequenceNumber() < m_tcb->m_rxBuffer->NextRxSequence() || tcpHeader.GetSequenceNumber() > m_tcb->m_rxBuffer->MaxRxSequence())
    {
      return;
    }
  ...
  TcpSocketBase::CalculateWScale() const
  {
    ...
    NS_LOG_INFO("Node " << m_node->GetId() << " calculated wscale factor of " << static_cast<int>(scale) << " for buffer size " << m_tcb->m_rxBuffer->MaxBufferSize());
    return scale;
  }
